...
5. **Code wins over docs (hard rule).** Documentation drifts; the codebase is the ground truth for **current behavior**. When `search_code` / `get_context` disagree with a Confluence/Jira/GDoc/PRD/BRD/ADR/runbook, treat the **code** as authoritative for "what the system does today" and the **doc** as authoritative for "what was intended". Always surface the gap explicitly in the artifact you are producing (e.g. PRD **Known Gaps**, plan **Constraints**, analyze **Findings**), citing both the code path and the doc URL — never silently parrot a stale doc.
